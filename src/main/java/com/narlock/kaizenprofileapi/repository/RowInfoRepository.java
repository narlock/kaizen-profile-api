...
public interface RowInfoRepository extends JpaRepository<RowInfo, RowInfoId> {
    @Modifying
    @Transactional
    @Query("INSERT INTO RowInfo (profileId, rowIndex, widgets) VALUES (:profileId, :rowIndex, :widgets)")
    void saveRowInfo(@Param("profileId") Integer profileId,
                     @Param("rowIndex") Integer rowIndex,
                     @Param("widgets") String widgets);

    @Modifying
    @Transactional
    @Query("UPDATE RowInfo r SET r.profileId = :profileId, r.rowIndex = :rowIndex, r.widgets = :widgets WHERE r.profileId = :profileId AND r.rowIndex = :rowIndex")
    void updateRowInfo(@Param("profileId") Integer profileId,
                     @Param("rowIndex") Integer rowIndex,
                     @Param("widgets") String widgets);
